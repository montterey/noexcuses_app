...
BEGIN
  ...
  UPDATE public.challenges SET status=new_status,starts_at=s,ends_at=s+make_interval(days=>v.duration_days),registration_ends_at=LEAST(COALESCE(registration_ends_at,s),s),accepted_at=COALESCE(accepted_at,now()) WHERE id=p_challenge_id;
