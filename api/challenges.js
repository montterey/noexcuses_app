...
export function validateCreateChallengeInput(value) {
  ...
  const targetValue = mode === 'first_to_target'
    ? positiveInteger(input.targetValue, 'targetValue')
    : null;
  const invitedUserId = optionalUuid(input.invitedUserId, 'invitedUserId');

  if ((visibility === 'private' || joinMode === 'invite_only') && !invitedUserId) {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (body.action === 'listMine') {
      ...
      const challenges = await callRpc(supabase, 'list_user_challenges', {
        p_user_id: userId,
        p_scope: scope,
      });
      return json(res, 200, { success: true, challenges });
